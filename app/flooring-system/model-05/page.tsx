...
const colorOptions = [
  { name: "White", src: "/images/model-05-white.jpg" },
  { name: "Black", src: "/images/model-05-black.jpg" },
  { name: "Blue", src: "/images/model-05-blue.jpg" },
  { name: "Green", src: "/images/model-05-green.jpg" },
];

const specifications = [
  { label: "Dimensions", value: "25 × 100 × 5 cm" },
  { label: "Weight", value: "1.6 kg / piece" },
  { label: "Performance", value: "Weather resistant · UV resistant · Long-lasting outdoor use" },
];

const applications = [
  { thai: "พื้นที่แคบ", english: "NARROW SPACES", description: "Designed for areas where standard wider panels may not fit as neatly." },
  { thai: "จัดสวน", english: "GARDEN USE", description: "Suitable for garden layouts and decorative outdoor spaces." },
  { thai: "ทางเดิน", english: "WALKWAYS", description: "Helps create cleaner and more practical walking paths." },
  { thai: "โรงเรือน", english: "GREENHOUSE", description: "Ideal for greenhouse flooring and plant-care work areas." },
  { thai: "รองกระถางต้นไม้", english: "UNDER POTS", description: "Keeps plant pots elevated, organized, and cleaner." },
  { thai: "ใช้งานร่วมกับรุ่น 25 × 25 × 5", english: "WORKS WITH 25 × 25 × 5 MODEL", description: "Can be used together with the smaller modular tile for more flexible layouts." },
];

export default function Model05Page() {
    ...
    <main id="top" className="min-h-screen bg-[#F7F6F1] text-[#26352C]">
        ...
        <div className="grid gap-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-start lg:gap-20">
          <div>
            <div className="flex aspect-[1.12/1] items-center justify-center overflow-hidden rounded-[28px] bg-white p-8 shadow-[0_20px_55px_rgba(54,82,64,0.08)] sm:p-14">
              <img src={selectedColor.src} alt={`Plastic Slat in ${selectedColor.name}`} className="h-full w-full object-contain" />
            </div>
          ...
          <div className="lg:pt-4">
            <p className="text-xs uppercase tracking-[0.26em] text-[#64806C]">Flooring System / Model 05</p>
            <h1 className="mt-5 text-5xl font-semibold leading-[0.95] tracking-[-0.05em] sm:text-7xl">Plastic Slat</h1>
            <p className="mt-6 text-2xl font-medium leading-tight text-[#476A55]">แผ่นปูพื้นจัดสวน</p>
            <p className="mt-8 max-w-xl text-base leading-8 text-[#68766C]">แผ่นปูพื้นจัดสวนขนาด 25 × 100 × 5 ซม. ออกแบบให้เหมาะกับพื้นที่ใช้งานที่แคบหรือพื้นที่ที่ต้องการความพอดีมากขึ้น แข็งแรง ใช้งานง่าย ระบายน้ำได้ดี ทนแดด ทนฝน และเหมาะสำหรับจัดสวน ทางเดิน โรงเรือน รองกระถางต้นไม้ และพื้นที่อเนกประสงค์</p>
            <p className="mt-4 max-w-md text-sm leading-7 text-[#8A948C]">A slim modular plastic flooring panel designed for narrower spaces, garden use, greenhouse areas, walkways, and everyday utility applications.</p>
      ...
      <section className="border-y border-[#26352C]/10 bg-[#EEF1EB]">
        <div className="mx-auto max-w-7xl px-6 py-12 lg:px-10 lg:py-16">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-[#64806C]">Made for everyday spaces</p>
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-[-0.04em] sm:text-4xl">A slimmer floor.<br />More flexible layouts.</h2>
          </div>
      ...
      <nav aria-label="Product navigation" className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-10 text-sm text-[#476A55] sm:flex-row sm:items-center sm:justify-between lg:px-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:gap-8">
          <Link href="/flooring-system/model-04" className="transition hover:text-[#304539]">Previous Model — Model 04</Link>
          <Link href="/flooring-system/model-06" className="transition hover:text-[#304539]">Next Model — Model 06 →</Link>
        </div>
      </nav>
    </main>
  );
