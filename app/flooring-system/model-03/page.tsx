...
const colorOptions = [
  { name: "White", src: "/images/model-03-white.jpg" },
  { name: "Black", src: "/images/model-03-black.jpg" },
  { name: "Blue", src: "/images/model-03-blue.jpg" },
  { name: "Green", src: "/images/model-03-green.jpg" },
];

const specifications = [
  { label: "Dimensions", value: "30 × 50 × 5 cm" },
  { label: "Weight", value: "700 g / piece" },
  { label: "Load capacity", value: "300 kg" },
  { label: "Performance", value: "Weather resistant · UV resistant · Long-lasting for outdoor use" },
];

const applications = [
  { thai: "ต่อกับรุ่น 2", description: "Extend or connect with Model 02 for more flexible layouts." },
  { thai: "จัดสวน", description: "Suitable for decorative garden flooring and neat space organization." },
  { thai: "รองกระถางต้นไม้", description: "Helps lift pots from the ground and keeps areas cleaner." },
  { thai: "พื้นที่แคบ", description: "Ideal for compact corners, narrow walkways, and small utility zones." },
];

export default function Model03Page() {
    ...
    <main id="top" className="min-h-screen bg-[#F7F6F1] text-[#26352C]">
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-10 lg:py-16">
        ...
        <div className="grid gap-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-start lg:gap-20">
          ...
          <div className="lg:pt-4">
            <p className="text-xs uppercase tracking-[0.26em] text-[#64806C]">Flooring System / Model 03</p>
            ...
            <p className="mt-8 max-w-xl text-base leading-8 text-[#68766C]">แผ่นปูพื้นจัดสวนขนาดกะทัดรัด เหมาะสำหรับใช้งานร่วมกับรุ่น 2 ช่วยต่อเติมพื้นที่เล็กหรือมุมแคบได้อย่างลงตัว ใช้สำหรับจัดสวน วางรองกระถางต้นไม้ และจัดพื้นที่ใช้งานให้เป็นระเบียบ ทนแดด ทนฝน และใช้งานได้ยาวนาน</p>
            <p className="mt-4 max-w-md text-sm leading-7 text-[#8A948C]">Compact modular plastic flooring designed for small spaces and extension use with Model 02. Suitable for garden styling, supporting plant pots, and narrow-area applications.</p>
      ...
      <section className="border-y border-[#26352C]/10 bg-[#EEF1EB]">
        <div className="mx-auto max-w-7xl px-6 py-12 lg:px-10 lg:py-16">
          <div>
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-[-0.04em] sm:text-4xl">Small format.<br />More possibilities.</h2>
          </div>
          <div className="mt-10 grid gap-6 text-sm leading-7 text-[#68766C] sm:grid-cols-2 lg:grid-cols-4">
            {applications.map((application) => (
              <div key={application.thai}>
      ...
      <nav aria-label="Product navigation" className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-10 text-sm text-[#476A55] sm:flex-row sm:items-center sm:justify-between lg:px-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:gap-8">
          <Link href="/flooring-system/model-02" className="transition hover:text-[#304539]">Previous Model — Model 02</Link>
          <Link href="/flooring-system/model-04" className="transition hover:text-[#304539]">Next Model — Model 04 →</Link>
        </div>
      </nav>
    </main>
  );
